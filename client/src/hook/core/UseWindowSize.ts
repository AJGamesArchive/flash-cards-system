...
/**
 * Hook to detect the window size of the device running the application and update these sizes if the screen is resized
 * @returns Width & Height of current device screen
 */
function useWindowSize(): WindowSize {
  ...
  useEffect(() => {
    // Create event handler to update the saved screen resolution numbers when the screen size is updated
    const handleResize = () => {
      setWindowSize({
        width: window.innerWidth,
        height: window.innerHeight,
