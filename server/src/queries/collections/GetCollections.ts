 ...
 */
async function getCollections(userUUID?: string, collectionUUID?: string): Promise<FullCollection[]> {
  let collectionQuery;
  ...
  return collectionQuery.map((collection) => ({
    ...collection,
    numSets: collection.collectionAllocation.length,
  } as FullCollection)) as FullCollection[];
};
