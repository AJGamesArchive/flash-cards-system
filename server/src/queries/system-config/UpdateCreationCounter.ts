// Imports
import { db } from '../../Server.js';
import SystemConfig from '../../types/SystemConfig.js';

/**
 * Async function to update the creation counter config in the DB
 * @param newCounter The new creation counter
 * @returns Flag to indicate success
 */
async function updateCreationCounterConfig(
	newCounter: number,
): Promise<SystemConfig | null> {
	try {
		const updatedConfig = await db.systemConfig.update({
			where: {
				configUUID: '7d6456e7-53f9-4d23-a547-a2590dd5bc30',
			},
			data: {
				creationCounter: newCounter,
			},
		});
		return updatedConfig;
	} catch (error: any) {
		console.error(error);
		return null;
	}
}

export default updateCreationCounterConfig;
