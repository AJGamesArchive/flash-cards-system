 ...
 */
async function saveFlashcardSet(set: FlashcardSet, flashcards: Flashcard[], newFlag?: boolean): Promise<number> {
  ...
  if(newFlag) {
    ...
    flashcards.forEach((flashcard) => queries.push(db.flashCards.create({
      data: {
