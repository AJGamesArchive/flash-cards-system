...
const routeSetsSetUUIDCards = async (
  req: FastifyRequest<{ Params: SetsSetUUIDCardsParams, Querystring: SetsSetUUIDCardsQuery }>,
  rep: FastifyReply
): Promise<void> => {
  ...
  // Return flashcards
  rep.status(200).send(flashcards.map((card) => ({
    ...card,
    createdAt: card.createdAt.toISOString(),
    updateAt: card.updatedAt.toISOString(),
  } as SetsSetUUIDCardsReply200)) as SetsSetUUIDCardsReply200[]);
  return;
};
