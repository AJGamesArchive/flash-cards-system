...
const routePOSTUserCollectionSetAllocations = async (
  ...
  const creationStatus: number = await createCollectionSetAllocation(req.params.userUUID, req.params.collectionUUID, req.body.setUUID);
  if(creationStatus !== 201) {
    rep.status(creationStatus).send({
      message: (creationStatus === 403) ?
