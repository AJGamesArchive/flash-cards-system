...
const routeGETUserCollectionSetAllocations = async (
  ...
  const sets: FullSet[] | null = await getCollectionSets(req.params.userUUID, req.params.collectionUUID);
  if(!sets) {
    rep.status(404).send({
  ...
  // Return sets
  rep.status(200).send(sets.map((set) => ({
    ...set,
    createdAt: set.createdAt.toISOString(),
    updatedAt: set.updatedAt.toISOString(),
  } as GETUserCollectionSetAllocationsReply200)) as GETUserCollectionSetAllocationsReply200[]);
  return;
};
