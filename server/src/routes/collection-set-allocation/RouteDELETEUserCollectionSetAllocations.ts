...
const routeDELETEUserCollectionSetAllocations = async (
  ...
  // Delete allocation
  const deleteStatus: number = await deleteCollectionSetAllocation(req.params.userUUID, req.params.collectionUUID, req.params.setUUID);
  if(deleteStatus) {
    rep.status(deleteStatus).send({
      message: (deleteStatus === 403) ?
