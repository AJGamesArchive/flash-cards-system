...
const routePATCHUsersUserUUID = async (
  req: FastifyRequest<{ Params: PATCHUsersUserUUIDParams, Body: PATCHUsersUserUUIDRequest }>,
  rep: FastifyReply
): Promise<void> => {
  // Map JWT data
  ...
  // Update user
  const updatedUser: User | null = await updateUser(req.params.userUUID, req.body.username, req.body.adminFlag);
  if(!updatedUser) {
    rep.status(404).send({
