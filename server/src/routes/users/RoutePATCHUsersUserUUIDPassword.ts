...
const routePATCHUsersUserUUIDPassword = async (
  req: FastifyRequest<{ Params: PATCHUsersUserUUIDPasswordParams, Body: PATCHUsersUserUUIDPasswordRequest }>,
  rep: FastifyReply
): Promise<void> => {
  // Map JWT data
  ...
  const updateStatus: number = await updatePassword(req.params.userUUID, req.body.oldPassword, req.body.newPassword);
  if(updateStatus !== 200) {
    rep.status(updateStatus).send({
      message: (updateStatus === 401) ?
