...
const routePOSTHiddenCardUserUUID = async (
  req: FastifyRequest<{ Params: POSTHiddenCardsUserUUIDParams, Body: POSTHiddenCardsUserUUIDRequest }>,
  rep: FastifyReply
): Promise<void> => {
  // Map JWT data
  ...
  const success: boolean = await createUserCardAllocation(req.params.userUUID, req.body.cardUUID);
  if(!success) {
    rep.status(404).send({
