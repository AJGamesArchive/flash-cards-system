...
const routeDELETEHiddenCardUserUUID = async (
  req: FastifyRequest<{ Params: DELETEHiddenCardsUserUUIDParams, Body: DELETEHiddenCardsUserUUIDRequest }>,
  rep: FastifyReply
): Promise<void> => {
  // Map JWT data
  ...
  const success: boolean = await deleteUserCardAllocation(req.params.userUUID, req.body.cardUUID);
  if(!success) {
    rep.status(404).send({
